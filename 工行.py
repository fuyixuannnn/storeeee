...
# def check_money(account,money):
#     if account in bank and float(money) > float(bank[account]["balance"]):
#         return 3
...
#             stat = check_money(account,money)
#             while True:
#                 if stat == 3:
#                     print("余额不足！")
#                 else:
...
def withdrawing():
    ...
    #             print("您当前余额为：￥",bank[account]["balance"])
    #             wd_amount = int(input("请输入取出的金额：￥"))
    #             if wd_amount <= bank[account]["balance"]:
...
def query():
    ...
    if account_number in bank:
        ...
        if pswd == bank[account_number]["password"]:
            ...
            开户行名称：%s
            '''
            print(info % (bank[account_number]["username"],account_number, bank[account_number]["password"], bank[account_number]["country"]+bank[account_number]["province"]+bank[account_number]["street"]+bank[account_number]["door"], bank[account_number]["balance"], bank[account_number]["bank_name"]))
        else:
            print("密码错误")
    else:
        print("该账号不存在！")
